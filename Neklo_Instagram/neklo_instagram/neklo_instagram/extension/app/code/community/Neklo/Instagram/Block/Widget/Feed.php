...
class Neklo_Instagram_Block_Widget_Feed extends Mage_Core_Block_Template implements Mage_Widget_Block_Interface
{
    ...
    public function getCacheKeyInfo()
    {
        ...
        $info[] = $designPackage->getTheme('locale')
            . '_' . $designPackage->getTheme('template')
            . '_' . $designPackage->getTheme('skin');
    ...
    {
        parent::setNameInLayout($name);

        $this->addData(
            array(
                'cache_lifetime' => $this->_getConfig()->getCacheLifetime(),
                'cache_key' => self::CACHE_KEY . '-' . $this->getCacheKey(),
            )
        );
    ...
    public function getDescription()
    {
        ...
        $hashtag = $this->getHashtag();
        $description = sprintf($description, $hashtag);
        return $description;
    }
